...
### 1.2 Meta 收购与 My Computer：从云端到桌面的范式跨越

> **2026年3月16日**，Meta 收购的 Manus AI 发布桌面应用 **My Computer**，将云端通用 Agent 延伸至用户本地机器。这是 Deep Agent 演进中的关键里程碑：**云端智能 + 本地执行**的混合架构正式落地。

**从云端沙箱到本地终端**：Manus 最初运行于云端隔离环境，具备网络、命令行、文件系统和浏览器，但无法直接访问用户本地资源（项目文件、开发环境、关键应用）。My Computer 通过 Desktop App 建立桥梁，让 Manus 能够直接操控本地终端。

**核心技术路径**：通过 `osascript`（macOS）和 CLI 命令在用户机器上执行操作——读取、分析、编辑本地文件，启动并控制本地应用程序。官方演示案例：
- 花店老板：说一句话"整理我的花店照片"，Manus 扫描图片内容，按类别创建子文件夹，自动分类数千张图片
- 会计：数百张发票重命名为标准格式，过去需要一整天，Manus 几分钟完成
- 开发者：完全通过终端命令在 Mac 上构建了一个实时会议翻译字幕 Swift 应用，全程未打开 Xcode，全程由 Manus 编写、调试、打包

**唤醒闲置算力**：My Computer 还解锁了本地 GPU 资源——用本地 GPU 训练 ML 模型或跑 LLM 推理；闲置的 Mac mini 可变身 24/7 AI 助手，无论身处何地都可向家中机器发送指令，Manus 在后台完成工作。

**云+本的混合架构**：Manus 云端保留 Google Calendar、Gmail 和第三方服务集成；现在叠加了本地文件访问能力，形成"文件在本地、邮件走云端、Manus 无缝桥接两者"的新工作流。这是 **AI 编排云端与本地资源融合**的首次大规模产品化实践。

### 1.3 Deep Agent 的技术特征
...
| 基准 | 评估内容 | 当前最高成绩 |
|------|---------|-------------|
| [GAIA](articles/research/gaia-osworld-benchmark-2026.md) | 真实世界问答、任务完成 | GPT-5 Mini: 44.8% / Claude 3.7 Sonnet: 43.9%（2026年3月最新） |
