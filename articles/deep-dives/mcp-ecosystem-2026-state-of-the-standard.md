...
## 关联补充：MCP 企业级价值重估 — 为什么"CLI 替代 MCP"是误导

> 以下内容从企业采纳视角补充 MCP 的不可替代价值。
...
| 行为可见性 | 随意 | 必须可审计 |
| 质量保障 | 靠个人检查 | 需可量化 Telemetry |

### MCP 被忽视的三大企业能力

1. **MCP Prompts & Resources**：企业知识对齐机制，结构化注入内部知识
2. **Auth & Security**：SSO/LDAP 统一身份认证，集中化权限控制
3. **Telemetry & Observability**：Agent 行为可观测性，CLI 的最大盲区
...
| 一次性临时任务 | Ephemeral MCP Runtime（按需启动/销毁） |

**核心结论**：MCP 的问题不是"过度设计"，而是它的价值只有在大规模组织采纳时才能体现。CLI 替代 MCP 的叙事是用个人开发者的尺子量企业采纳的需求。

> 来源：http://chrlschn.dev/blog/2026/03/mcp-is-dead-long-live-mcp/

---

## 关联补充：MCP 基础入门 — 为什么需要 MCP

> 以下内容作为 MCP 基础概念的补充参考。
...
**MCP 的核心价值**：一次编写，处处运行——标准化的协议层替代了逐个定制的集成代码。
