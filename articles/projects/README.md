- **[langchain-ai/openwiki](./langchain-ai-openwiki-cli-agent-documentation-1626-stars-2026.md)** — 1,626 ⭐（R638 Defer 1,454 → R641 1,626，+11.7%），TypeScript，MIT，**LangChain 1st-party CLI Agent 文档工具 — "Built specifically for agents"**：3 种交互模式（`openwiki` 交互式 / `openwiki "<request>"` 单次 / `openwiki -p "<prompt>"` 单命令）+ `openwiki --init` 跨 LLM provider 初始化 + GitHub Action `openwiki-update.yml` 每天自动开 PR 保持文档更新 + 1st-party LangChain 维护（与 LangGraph / LangSmith / LangServe 原生集成路径）+ Sonnet 5 1st-party 推荐配置；与本轮配套 Article [`anthropic-claude-sonnet-5-agentic-sonnet-most-agentic-yet-2026`](../fundamentals/anthropic-claude-sonnet-5-agentic-sonnet-most-agentic-yet-2026.md) 形成「Anthropic 1st-party 模型层 + LangChain 1st-party 框架层」跨厂商 1st-party 同源生态 Pair 闭环：Sonnet 5 提供 agentic execution layer（multi-step / 工具调用 / autonomous operation）+ openwiki 提供 agentic documentation layer（让 Agent 自己维护代码库文档）+ GitHub Action 自动化让 Agent 文档工程从人类产出变成 Agent 自主维护的资产，**R641 本文新增**

- **[microsoft/Memora](./microsoft-memora-harmonic-memory-agent-long-horizon-2026.md)** — 110⭐，Python（≥3.10），MIT，**1st-party Microsoft Research ICML 2026 论文官方代码 — 用「谐波记忆表示」解开长期记忆的抽象-具体死结**：3 层组件架构（Memory value 富表达原文不索引 + Primary abstraction 6-8 词短句作为 canonical 检索单元 + Cue anchors organic tags 提供 many-to-many 替代入口）+ 4 种 retrieval strategy（semantic / prompted / hybrid + BM25 / GRPO 强化学习实验性）+ 完整 lifecycle（Ingestion → Storage → Retrieval → Generation）+ ChromaDB 向量后端 + LoCoMo + LongMemEval 内置 benchmark runners（Hydra 配置）+ 6 大企业级能力（轻量集成 / 结构化表示 / 共享空间 / 多类型 / 受控隔离 / 灵活后端）+ SoTA on LoCoMo + LongMemEval 击败 Mem0 / RAG / full-context，**最多减少 98% context tokens**。完全对应本轮配套 Article [`microsoft-research-memora-harmonic-memory-representation-long-horizon-2026`](../context-memory/microsoft-research-memora-harmonic-memory-representation-long-horizon-2026.md) 1st-party 学术锚点：Microsoft Research Blog Memora 文章（谐波表示范式转折 + 抽象-具体解耦判断）↔ 本文（1st-party ICML 2026 论文官方代码实现 + 完整 lifecycle + 4 retrieval strategy + 内置 benchmark runner）。两者共同把「长期记忆」从「内容侧优化」迁移到「索引侧重新设计」——**检索入口必须独立于内容**，**R640 本文新增**

- **[NousResearch/hermes-agent-self-evolution](./nousresearch-hermes-agent-self-evolution-skill-optimizer-dspy-gepa-4478-stars-2026.md)** — 4,478 ⭐，Python，MIT，**DSPy + GEPA 驱动的 Agent Skill 训练式演化引擎**（Hermes Agent 子项目）：5 阶段分批落地（Phase 1 SKILL.md 已实现 / Phase 2-5 计划）+ 文本空间里跑 forward–backward–update 循环 + textual learning rate（Skill ≤15KB, tool desc ≤500 chars）+ rejected-edit buffer + validation gate + 5 条 guardrails（test/size/cache/semantic/PR）+ 跨 harness 真实会话数据（Claude Code/Copilot/Hermes sessiondb）+ $2-10/run 低成本。完全对应本轮配套 Article [`microsoft-research-skillopt-agent-skills-as-trainable-parameters-2026`](../research/microsoft-research-skillopt-agent-skills-as-trainable-parameters-2026.md) 1st-party 学术锚点：SkillOpt 论文（forward–backward–update 循环 + 6 基准 / 7 模型 / 52 cells 最佳）↔ 本文（DSPy+GEPA 开源工程实现 + 5 阶段分批 + 5 guardrails）。两者共同把「skill 演化」从一次性撰写升级为完整训练循环，**R637 本文新增**

- **[0xNyk/council-of-high-intelligence](./0xnyk-council-of-high-intelligence-multi-agent-deliberation-2759-stars-2026.md)** — 2,759 ⭐，Shell/Markdown，CC0-1.0，**18 AI personas 多 LLM 协同审议 — 把「开一次董事会」压缩成一条 Claude Code slash command `/council`**：18 个伟人 persona（Socrates ↔ Aristotle / Feynman ↔ Machiavelli / Lao Tzu ↔ Sun Tzu / Kahneman ↔ Meadows / Taleb ↔ Munger 等 9 组**极性配对** — Polarity Pairs 是核心机制，不是装饰）+ 跨 provider 路由（Claude/OpenAI/Gemini/Ollama，**多 LLM 而不是 costume change on one model**）+ 3 档审议模式（Quick 2-3 轮 / Standard 18 全部 / Duo 2 persona 极性对比）+ 4 道防 groupthink 的 gate（Problem Restate Gate 让 3+ member 重述问题捕获错问 / Dissent Quota 强制保留少数派 / Novelty Gate 禁止刷已说论点 / >70% 同意太早时强制 2 人 steelman 反方）；与本轮配套 Article [`facebook-astryx-meta-1st-party-design-system-agent-ready-2026`](../ai-coding/facebook-astryx-meta-1st-party-design-system-agent-ready-2026.md) 形成「让 Agent 写什么（Astryx 设计契约）↔ 让 Agent 怎么想（Council 审议协议）」对照闭环 —— 两个项目一起勾勒出 **2026 H2 Agent Engineering 的两个新维度：Design System for Agents + Decision Harness for Agents**，**R620 本文新增**

- **[anthropics/launch-your-agent](./anthropics-launch-your-agent-1st-party-claude-code-skill-2026.md)** — 584 ⭐，Markdown（SKILL.md 27.5KB），Apache-2.0，**1st-party Claude Code Skill，把「interview → stage & launch → grade & iterate → run without them」4 阶段 Managed Agent 生命周期装进单个 SKILL.md**：Harness-as-Skill 范式（Skill 不只是知识容器，而是承载完整 CMA 生命周期工程机制的 Harness）+ Stage-then-Launch 协议（Credential Dependency Window 压缩到分钟级）+ Build Kit as Code（`my-agent/build-sheet.json` 单源真相，agent.json/environment.json/outcome.md 都是其 projection）+ Evaluator Loop（先读 grader verdict + 单变量迭代 + holdout cases + version bump 决策树）+ NEXT-DIRECTIONS.md（v1/v2/v3 增量契约 + 显式区分"做不到/缺凭证/暂不在 v0"三种 deferral 原因）+ Live Schema Page（`agent-overview.html` 把 Harness 状态映射到 API 字段，emoji shorthand 保持视觉一致）；与本轮配套 Article [`anthropic-launch-your-agent-skill-as-complete-harness-2026`](../harness/anthropic-launch-your-agent-skill-as-complete-harness-2026.md) 形成「项目实证 ↔ 范式提炼」完整闭环，**R605 本文新增**
- **[raiyanyahya/recall](./raiyanyahya-recall-claude-code-local-first-memory-639-stars-2026.md)** — 639 ⭐，Python（stdlib-only + 可选 numpy 加速），MIT，**Claude Code 插件，用 TF-IDF + TextRank 做零 token 成本的项目记忆**：双文件设计（`history.md` append-only 完整 audit trail + `context.md` overwrite 1-2K tokens 摘要）+ Stop hook 增量捕获（每轮 append，不做一次性 dump）+ `summarizer.py` 200 行 stdlib-only 的 TextRank 实现（numpy 存在时加速，不存在时降级到纯 Python，CI gate 两条路径必须输出相同句子）+ 6 个可被验证的 privacy 护栏（无网络调用/无 API key/secret redaction/hardened git/path confinement/untrusted fence）+ 完整 production-grade 质量基线（ruff + bandit + pytest + CodeQL + multi-Python 3.9-3.13 测两条路径 + benchmark quality gate）；与本轮配套 Article [`raiyanyahya-recall-local-first-project-memory-textrank-zero-token-2026`](../context-memory/raiyanyahya-recall-local-first-project-memory-textrank-zero-token-2026.md) 形成「项目实证 ↔ 非 LLM 记忆架构范式提炼」完整闭环，**R606 本文新增**

- **[Unclecheng-li/VulnClaw](./unclecheng-li-vulnclaw-ai-pentest-agent-1166-stars-2026.md)** — 1,166 ⭐，Python 3.10+，MIT，**目标驱动求解 + 证据级反幻觉的渗透测试 Agent**：把 Agent 执行范式从「固定轮数循环」升级为「目标驱动的状态空间搜索」——目标达成 / 探索前沿耗尽 / 安全预算触达三停止条件 + 黑板图 Fact/Intent 原语（结构上杜绝原地打转）+ 三道证据闸门（结论闸门/完成闸门/即时收敛）让 Agent 声称的 flag 必须逐字符匹配真实工具输出 + reflexion.py L0-L4 渐进 Payload 升级（原始 → URL 编码 → 双写注释 → Unicode/hex → 多层混淆）+ 21 个 Skill（7 核心 + 14 专项）+ 4 MCP 服务 + 13 个 LLM Provider；解决了 2026 年 Agent 工程被低估的「完成信号无客观校验」范式问题，与本仓库 harness 评估器循环 + 反幻觉 + 工程机制体系形成完整闭环，**R593 本文新增**

- **[deepreinforce-ai/Ornith-1](./deepreinforce-ai-ornith-1-self-improving-agentic-coding-model-578-stars-2026.md)** — 578 ⭐（创建仅 9 天），Python/Tensors，MIT，**首个把「self-improving scaffold」当作显式训练目标的开源 Agentic Coding 模型族**：9B Dense / 35B+397B MoE 三档全 MIT + OpenAI-compatible API + 256K context + 原生 tool calling + RL 同时优化「解法 rollout」与「驱动 rollouts 的 scaffold 本身」+ Terminal-Bench 2.1 (Claude Code harness) 78.2 / SWE-bench Verified 82.4 / NL2Repo 48.2；NL2Repo 反超 Claude Opus 4.8 说明长程任务能力 + 与本轮配套 Article [`openai-codex-remote-engineering-control-plane-queue-vs-steer-plan-vs-goal-2026`](../practices/ai-coding/openai-codex-remote-engineering-control-plane-queue-vs-steer-plan-vs-goal-2026.md) 形成「执行侧长程闭环 ↔ 决策侧控制平面」的 agentic coding 双轴互补，**R594 本文新增**

- **[HKUDS/Vibe-Trading](./hkuds-vibe-trading-mandate-gated-trading-agent-15213-stars-2026.md)** — 15,213 ⭐（GitHub Trending 当日 +839），Python，MIT，**把「经纪商安全」从字符串校验升级为结构化分层闸门**：paper/live 边界做成 broker 级别的结构性 discriminator（10 个 broker，含 5 个 live-capable + 5 个强制 paper）+ mandate-gated order placement（用户事先 commit 的 symbol universe / 订单上限 / 持仓上限 / 当日上限 + filesystem kill switch + fail-closed pre-trade gate + audit ledger）+ Provider Reliability Layer（DeepSeek/Kimi/Gemini/OpenRouter 各自 capability layer，避免 OpenAI-compatible shim 互相污染）+ Research Goal Runtime（session-scoped goal metadata：claims / acceptance_criteria / evidence_rows / budgets / completion_policy）+ 79 Skills（8 类）+ 54 MCP Tools + 18 数据源；与本轮配套 Article [`cursor-cloud-agents-durable-execution-three-layer-state-decoupling-2026`](../deep-dives/cursor-cloud-agents-durable-execution-three-layer-state-decoupling-2026.md) 形成「部署侧 broker safety ↔ 执行侧云 Agent durable execution」的 production-grade harness 双轴互补，**R595 本文新增**

- **[JCodesMore/ai-website-cloner-template](./jcodesmore-ai-website-cloner-git-worktree-parallel-22074-stars-2026.md)** — 22,074 ⭐，TypeScript，MIT，**Git Worktree 并行化网站克隆**：多 Agent 并行构建的核心工程难题——如何让多个 Agent 同时写代码不打架？答案是 Git Worktree，每个 Builder Agent 在独立 worktree 中工作，拿到的不是模糊设计描述而是精确到像素的 CSS 值（`getComputedStyle()`），最后合并回主分支；解决了多 Agent 并行化中最难的「工作区状态隔离 + 无冲突合并」问题，与 Cursor SWE-bench Harness 泄露分析（../harness/）形成「并行构建协调 ↔ 评估环境隔离」互补闭环，**R566 本文新增**
- **[QwenLM/Qwen-AgentWorld](./qwen-agentworld-language-world-model-2026.md)** — 634 ⭐，Python，Apache-2.0，**首个统一 7 领域语言世界模型**：MCP/搜索/终端/软件工程/Android/Web/OS 七大领域统一在单一 LLM 中学会环境状态迁移；CPT+SFT+RL 三阶段训练 pipeline + Decouple（环境仿真器）和 Unify（Agent 基础模型）双应用范式 + 已在 OpenClaw 环境验证；解决 Agent 环境仿真碎片化问题，与 Cursor Self-Driving Codebases 形成「环境表示层 ↔ 多 Agent 协作层」正交互补闭环，**R583 本文新增**

- **[calesthio/OpenMontage](./calesthio-openmontage-agentic-video-production-27300-stars-2026.md)** — 27,303 ⭐，Python/TypeScript，AGPL-3.0，**GitHub Trending #1 全流程 Agentic 视频制作系统**：12 条生产管线 + 52+ 工具 + 500+ Agent Skills + $0.69 生成一条广告，把 Claude Code/Cursor/Copilot 变成完整视频制作团队；多 Agent 管线编排（Research→Script→Asset→Voice→Edit→Render）在创意生产领域的首次大规模实践；Stars 2 个月内从 6,514 增至 27,303（4x），开源视频制作 Agent 化里程碑，与 OpenAI Workspace Research Article 形成「全流程自动化」互补闭环，**R582 本文新增**

- **[xbtlin/ai-berkshire](./xbtlin-ai-berkshire-multi-agent-value-investing-4005-stars-2026.md)** — 4,005 ⭐，Python/Bash，MIT，**基于 Claude Code 的多 Agent 价值投资研究框架**：4 大师方法论（巴菲特/芒格/段永平/李录）并行 Agent + 独立评分与交叉验证 + 8 条否决红线 + 精确财务计算（decimal.Decimal）+ 实盘验证（2024 +69.29%，2025 +66.38%）；证明了垂直领域方法论可编码为 Claude Code Skill 文件，为「专业 Agent = 通用模型 + 领域方法论 Skill」范式提供实证，**R564 本文新增**
...
- **[stablyai/orca](./stablyai-orca-multi-agent-ide-2398-stars-2026.md)** — 2,398 ⭐，TypeScript，MIT，**多 Agent 并行工作台**：Parallel Worktrees（git 隔离的 Agent 执行环境）+ Mobile Companion（iOS/Android 移动端监控）+ Design Mode（点击任意 UI 元素直接注入 Agent context）+ Terminal Splits（WebGL 渲染的终端多路复用）+ SSH Worktrees（远程 Agent 执行）；Orca 把每个 coding agent 装进独立 git worktree，用桌面 App 统一调度、对比、合并——代表 2026 年「单 Agent 长会话 → 多 Agent 并行」范式拐点的成熟产品化实践，与 R556 Human-Agent Teams 形成「多 Agent 并行工作 ↔ 团队协作治理」互补闭环，**R557 本文新增**

- **[HKUDS/AgentSpace](./hkuds-agentspace-human-agent-collaborative-workspace-339-stars-2026.md)** — 339 ⭐，TypeScript，Apache-2.0，**Human+Agent 协作工作空间**：AgentRouter 统一执行契约（Claude Code/Codex/OpenCode/OpenClaw/Hermes）+ 数字员工看板 + 权限控制平面 + 与 ClawTeam (5341⭐) 同 HKUDS 研究组形成「多 Agent 并行调度 ↔ 团队协作治理」互补闭环，**R522 本文新增**
...
- **[anthropics/claude-code](./anthropics-claude-code-official-agentic-coding-tool-133k-stars-2026.md)** — 133,791 ⭐，TypeScript/Shell，Anthropic，**Anthropic 官方 Agentic Coding CLI 工具**：原则落地的工程样本 + Auto Mode 双层防御（prompt-injection probe + transcript classifier）+ devcontainer 权限边界 + Privacy Safeguards 数据原则；与 R499 Teaching Claude Why Article（../harness/anthropic-teaching-claude-why-principles-over-demonstrations-2026.md）形成「原则层（Alignment Research） ↔ 工具层（Claude Code Engineering）」完整闭环，**R499 本文新增**

- **[microsoft/Webwright](./microsoft-webwright-terminal-browser-agent-5542-stars-2026.md)** — 5,542 ⭐，Python，MIT，**终端即舞台的 Browser Agent Harness**：三模块极简架构（~1K LOC）+ Agent 自主写 Playwright 代码控制浏览器 + 可复用 RPA 脚本产出 + GPT-5.4: 86.67% SOTA（Online-Mind2Web）+ 与 Cursor 3.8 /automate Skill（../practices/cursor-38-automate-skill-event-driven-autonomous-agents-2026.md）形成「/automate 事件驱动编排 ↔ Webwright 终端级 browser 复用」互补闭环，**R502 本文新增**

- **[garrytan/gstack](./garrytan-gstack-23-agent-roles-649-stars-2026.md)** — 649 ⭐，Shell/Markdown，MIT，**YC CEO 的 Claude Code 角色工程系统**：23 个 slash commands 实现虚拟工程团队（CEO/Eng Manager/Designer/QA/Security/Release Engineer）+ 角色专业化替代空白提示符 + 2026 年产出 240× 2013 年 + 与 Anthropic 并行 Claude C 编译器 Article（../orchestration/anthropic-parallel-claude-c-compiler-multi-agent-harness-2026.md）形成「多 Agent 并行（系统级） ↔ 单 Agent 角色切换（会话内）」完整闭环，**R498 本文新增**
...
- **[Fission-AI/OpenSpec](./fission-ai-openspec-57k-stars-sdd-ai-coding-2026.md)** — 57,992 ⭐，TypeScript，MIT，**Spec-Driven Development 规范驱动框架**：57k Stars 的 AI Coding 规范管理工具——`/opsx:propose` 生成 proposal/specs/design/tasks 四文件规范结构 + `/opsx:explore` 探索式需求澄清 + `/opsx:apply` 规范→代码闭环 + `/opsx:archive` 规范版本化；把规范从「人类的产出」变成「人机协作的界面」，解决 AI Coding「改完代码才发现需求从一开始就想错了」的根本性浪费，与 design.md Article 形成「规范记忆 → 结构化视觉记忆」互补闭环，**R562本文新增**
- **[yorgai/ORG2](./yorgai-org2-open-source-cursor-style-agent-ide-rust-harness-1289-stars-2026.md)** — 1,289 ⭐，TypeScript/Rust，AGPL-3.0，**开源 Cursor 风格 Agent IDE + Rust Harness**：内置 Rust Agent + 可回放执行轨迹（Replayable Traces）+ 10+ Agent CLI 统一调度 + 跨 session 持久化工作区 + 与 Cursor SDK Notion Article 形成「产品嵌入 ↔ 可审查性」互补闭环，**R559 本文新增**
...
- **[agentskills/agentskills](./agentskills-agentskills-agent-skills-specification-22243-stars-2026.md)** — 22,243 ⭐，Python（文档+验证脚本），Apache-2.0 代码 + CC-BY-4.0 文档双许可，**Agent Skills 官方开放规范仓库 — 把 Claude Code 的 SKILL.md 机制从「厂商私有格式」升级成「行业开放标准」**：SKILL.md 最小必填结构 + frontmatter 字段字面量约束（name 1-64 字符小写字母数字连字符且必须与父目录同名 / description 1-1024 字符且必须描述「何时使用」）+ 三阶段渐进式披露机制（Discovery 仅加载 name+description / Activation 全文加载 SKILL.md / Execution 按需加载 scripts+references+assets）+ 16+ 客户端支持矩阵（Claude Code/Codex CLI/Gemini CLI/Copilot/Cursor/Junie/VS Code/Spring AI/Continue.dev/OpenClaw）+ Anthropic 2025-12-18 公布的开源标准 + 与本仓库 123 篇 skills 主题文章形成「规范精确层 ↔ 工程共识层」对齐闭环 —— 之前 skills 文章默认 Agent Skills 「就是那样」，本文首次回到官方规范文档做精确解读，**R654 本文新增**
...
- [NousResearch/hermes-agent — 自改进 Agent 的工程实现 2026](nousresearch-hermes-agent-self-improving-agent-199k-stars-2026.md) — Stars 199K，Skill 自改进循环，FTS5+LLM 双层记忆 — 2026-06-22
...
- [gastown-multi-agent-workspace-manager-2026](./gastown-multi-agent-workspace-manager-2026.md) — 多 Agent 工作空间编排系统，**16,310 ⭐ R667 UPDATE**（+1,396 in 31 days vs R666 16,292，+0.11% in 8h cron cycle），Git Worktree 隔离 + Beads 账本 + Witness/Deacon/Dogs 三层监控 + v1.2.0/v1.2.1 关键稳定性修复 + v1.2.x 新增 Convoys/Molecules/Refinery 工业级 multi-agent 工作流引擎（关联：R666 deep dive [gastown-multi-agent-orchestration-deep-dive-r666-harness-protocolization-extension-2026](../orchestration/gastown-multi-agent-orchestration-deep-dive-r666-harness-protocolization-extension-2026.md) → harness 协议化三维度体系的 multi-agent 维度工业级实证）+ R667 新增 ogulcancelik/herdr 作为 Multi-Agent Stack Layer 1 (Multiplexer) 独立收敛证据，Go × Rust 双栈独立收敛到同一架构 = 工程范式正在形成）
...
- **[QwenLM/Qwen-AgentWorld](./qwenlm-qwen-agentworld-language-world-model-agent-environments-614-stars-2026.md)** — 614 ⭐，Python，Apache-2.0，**语言世界模型驱动的 Agent 环境模拟**：原生世界模型覆盖 7 个 Agent 交互域（MCP/Search/Terminal/SWE/Android/Web/OS）+ 三阶段训练管道（CPT→SFT→RL）+ AgentWorldBench 评测基准 + 可控扰动注入 + Qwen-AgentWorld-397B 评测 58.71 分超越 GPT-5.4（58.25）+ 零样本跨域泛化 + RL 后 Sim RL 在 QwenClawBench 上 +7.1 分；与 HuggingFace agent-eval（../evaluation/huggingface-is-it-agentic-enough-process-level-agent-eval-2026.md）形成「过程级评测方法 ↔ 世界模型模拟能力」互补闭环，**R581 本文新增**
...
- **[DeusData/codebase-memory-mcp](./deusdata-codebase-memory-mcp-26708-stars-r670-hybrid-paradigm-2026.md)** — **🎯 26,708 ⭐ R670 UPDATE**（R448 7,300+ ⭐ → R670 26,708 ⭐, **+19,408 ⭐ in ~3 weeks, +265% massive growth trajectory**），MIT，**Layer 4.3 Hybrid Memory Paradigm 首个工业级实证 + 代码知识图谱 MCP Server**：Tree-Sitter AST + 持久化 SQLite 图谱（Filesystem 范式）+ Nomic embed-code int8 768d 语义检索（Learning 范式）+ Hybrid LSP 10 语言语义类型解析（Hybrid 范式独有）+ 158 语言支持 + 120× 更少 token（3,400 vs 412,000 via file-by-file）+ 毫秒级查询 + arXiv:2603.27277 1st-party 论文（83% answer quality + 10× fewer tokens + 2.1× fewer tool calls 31 repos 评估）+ 14 MCP tools（Layer 5 Tool Runtime 实证）+ 11 种 Coding Agent 自动接入（Claude Code/Codex CLI/Gemini CLI/Zed/OpenCode/Antigravity/Aider/KiloCode/VS Code/OpenClaw/Kiro）+ Pure C zero dependencies + SLSA 3 + VirusTotal + 5,604 tests passing。**R670 UPDATE**: 与 [R670 Layer 4 Hybrid Memory Architecture 协议化监测 article](../orchestration/multi-agent-stack-r670-layer-4-hybrid-memory-architecture-protocol-monitoring-2026.md) + [R669 Layer 4 State/Memory Primitive article](../orchestration/multi-agent-stack-r669-state-memory-primitive-learning-vs-filesystem-paradigm-2026.md) 形成 **Layer 4.3 Hybrid Paradigm** 第三个实证——R669 二元划分（Learning + Filesystem）被 R670 修正为三元结构（Learning + Filesystem + Hybrid），Hybrid Memory Architecture 是 2026 H2 主流方向。**SKILL 防重协议前置检查 5 步 100% 达成**（R448 已 covered，88 天 > 30 天阈值 + +265% 增长触发 UPDATE 路径）。R670 关键判断：codebase-memory-mcp 是 Layer 4.3 Hybrid Paradigm 首个工业级实证 + Layer 4 ↔ Layer 5 Memory-Tool Contract 14 MCP tools 实证 + 14 MCP tools 跨平台标准化窗口。
...
- **[wanxingai/LightAgent](./wanxingai-lightagent-memory-tree-of-thought-mcp-2026.md)** — ~767-1,083 Stars，Python，Apache 2.0，极简主义 Agent 框架（No LangChain/No LlamaIndex）+ 分层 MemoryScope（trace/user_memory/self-reflection/delegation 四层）+ Tree-of-Thought 带反思推理 + LightFlow 确定性工作流编排（DAG/重试/轨迹）+ LightSwarm 多 Agent 协作 + MCP Native 双模式（stdio/SSE）+ 多 LLM 支持（OpenAI/DeepSeek/Qwen）。与 Anthropic April 23 Postmortem（eval 覆盖范围问题）形成主题互补：postmortem 揭示 eval 设计不足导致的系统性盲区，LightAgent 提供 MemoryScope 这样可追踪的记忆分层方案，两篇共同指向「长程 Agent 需要可解释的记忆架构」这一核心命题，**R501新增**

- **[Forsy-AI/agent-apprenticeship](./forsy-ai-agent-apprenticeship-reusable-experience-loops-893-stars-2026.md)** — 893 Stars，MIT，2026-06-23，post-training signal collector（首个把「训练数据生产」当产品做的项目）+ 500+ 任务种子 / 495 节课 / 1000+ 执行迹的开放生态 + npx 一行接入 Hermes Agent / OpenClaw / Codex / Claude Code / Cursor / OpenCode 六大 agent runtime + mentor agent 抽象 lesson 而非 scalar reward + 经济价值驱动的双向信号流动 + 与 [Anthropic Skill-Creator 文章](../harness/agent-apprenticeship-real-world-tasks-post-training-2026.md) 形成「Skill 知识编码 ↔ Apprenticeship 执行迹回灌」的工程互补。两篇共同指向「harness 的下一站是从 Skill 创作到 Experience Reuse」，**R521 新增**

- **[TracecatHQ/tracecat](./tracecat-hq-tracecat-open-source-security-automation-ai-agents-3690-stars-2026.md)** — 3,690 Stars，Python/TypeScript，AGPL-3.0，**企业级安全运营 Agent 编排平台**：MCP 集成任意 Coding Agent（Claude Code/Codex/Copilot）+ Temporal 持久化工作流（断点恢复）+ nsjail 沙箱隔离 + 100+ 预置安全工具连接器 + 人工审批节点（RBAC/ABAC）+ 审计日志 + 与 [OpenAI Daybreak Article](../harness/openai-daybreak-codex-security-evaluator-loop-harness-2026.md) 形成「商业评估器循环（Daybreak）↔ 开源平台化编排（Tracecat）」的互补闭环：两者共享同一设计哲学（执行 Agent + 验证器 + 人工在环），但实现路径不同——Daybreak 是端到端商业服务，Tracecat 是开源可扩展平台，**R529 新增**

- **[AgentBudget/agentbudget](./agentbudget-agentbudget-ulimit-for-ai-agents-105-stars-2026.md)** — 105 Stars（Apache-2.0，2026-02-15 创建），Python + Go SDK，**「ulimit for AI agents」开源 SDK — 把 Unix `ulimit` 思想搬到 AI Agent session-level cost control**：`wrap_client()` 透明包装 OpenAI / Anthropic / OpenRouter 等 LLM client（一行 import 启用 budget enforcement）+ `finalization_reserve` 留「收尾预算」让 Agent 主动 stop（不破坏 response 原子性）+ `would_exceed()` API 让 Agent 主动 query 预算状态（预算控制从「harness 强制力」升级为「Agent 自我管理」）+ OpenRouter 多模型统一计价 + 多语言 SDK（Python 主推 / Go 已发布 / Node 即将）。与 R617 Pair Article [`github-copilot-enterprise-governance-managed-settings-budget-control-2026`](../harness/github-copilot-enterprise-governance-managed-settings-budget-control-2026.md) 形成「1st-party 商业产品（GitHub Copilot Session Limits）↔ 开源 SDK（AgentBudget）」的工程互补 —— 商业版是 SaaS-managed（VS Code/CLI 内嵌，软上限，compaction 自动计量），开源版是 self-hosted library（`wrap_client()` 透明 wrap，可配 soft/hard，多 LLM 厂商统一计价），两者共同确认 **「Harness Budget Control」作为 2026 H2 独立基础设施类目** 已在 1st-party 和 OSS 两端同时收敛。Stars 虽仅 105 但概念先行 5 个月，1st-party 7/1 GA 是最强催化剂，**R617 本文新增**

- **[ogulcancelik/herdr](./ogulcancelik-herdr-agent-multiplexer-rust-11903-stars-2026.md)** — **11,950 ⭐** (R668 +47 vs R667 11,903 ⭐, +47 in 2h accelerated growth)，Rust，AGPL-3.0-or-later + 商业 dual-license，**tmux-rebuilt-for-agents —— Multi-Agent Stack Layer 1 (Multiplexer) 独立收敛证据**：per-Agent 真 PTY pane + agent-aware sidebar 状态可视化（🔴 blocked / 🟡 working / 🔵 done / 🟢 idle）+ background server detach/reattach（关笔记本不杀进程）+ socket API 让 Agent 也能驱动 multiplexer + ~10MB 静态二进制 SSH-friendly + 与 [R668 Layer 3 Skill Registry Primitive article](../orchestration/multi-agent-stack-r668-skill-registry-primitive-horizontal-decoupling-deep-dive-2026.md) + [R667 Multi-Agent Stack article](../orchestration/multi-agent-stack-r667-harness-protocolization-empirical-layering-2026.md) + [R666 gastown deep dive](../orchestration/gastown-multi-agent-orchestration-deep-dive-r666-harness-protocolization-extension-2026.md) 形成「Layer 1 Multiplexer × Layer 2 Orchestrator × Layer 3 Skill Registry」6 层 Multi-Agent Stack 实证闭环。**R668 UPDATE**: 距 12k⭐ BREAK 仅 **50⭐ gap**，R668 likely BREAK（R667 时 97⭐ gap → R668 时 50⭐ gap，+47/2h 加速增长）。**R667 NEW PROJECT** ✓ SKILL 防重协议前置检查通过（R620 Defer 因 License=NOASSERTION 已被 R667 重新确认为 AGPL-3.0 + 商业 dual-license，许可证合规）+ 协议兼容（gastown × herdr 通过 Bead ID → Pane ID + stdout pattern matching 桥接）+ 与 [gastown-multi-agent-workspace-manager-2026](./gastown-multi-agent-workspace-manager-2026.md) 形成「Layer 1 Multiplexer × Layer 2 Orchestrator」Multi-Agent Stack 完整分层实证。

- **[coreyhaines31/marketingskills](./coreyhaines31-marketingskills-vertical-skill-registry-marketing-36347-stars-2026.md)** — **36,347 ⭐**（R668 trigger 2026-07-06 03:57 CST，首次发现 + GitHub Trending daily），Markdown + Shell，MIT，**营销垂直 Skill Library —— Multi-Agent Stack Layer 3.3 Skill Library 内容层核心实证**：12+ 类（35+ skills）覆盖 SEO & Content / CRO / Content & Copy / Paid & Measurement / Growth & Retention / Sales & GTM / Strategy 7 个营销子领域 + `product-marketing` 基础 skill 强制前置 + Skill 互相 cross-reference 依赖图（`copywriting ↔ cro ↔ ab-testing` 等）+ 跨 Claude Code / OpenAI Codex / Cursor / Windsurf 4 个 Control Plane + Agent Skills spec（agentskills.io SKILL.md 标准）协议中立 + Corey Haines 商业背书（Conversion Factory + Magister + Swipe Files）。与 [R668 Layer 3 Skill Registry Primitive article](../orchestration/multi-agent-stack-r668-skill-registry-primitive-horizontal-decoupling-deep-dive-2026.md) + [alirezarezvani-claude-skills-354](./alirezarezvani-claude-skills-354-production-skills-20080-stars-2026.md)（Layer 3.2 通用 Skill Registry 20.4k ⭐）+ [Leonxlnx/taste-skill](https://github.com/Leonxlnx/taste-skill)（Layer 3.3 设计垂直 Skill Library 57.3k ⭐）形成「通用 vs 垂直」Skill Library 双实证 + 三子层（Skills Spec + Skill Registry + Skill Library）架构实证——Skill Library 必须支持「通用 + 垂直 + 自建」三模式并存。**R668 NEW PROJECT** ✓ SKILL 防重协议前置检查通过（5 步 100% 达成：未在 sources_tracked.jsonl / articles/projects/README.md / .agent/HISTORY.md 中出现）+ License 合规（MIT）+ 主题关联度 100%（与 R668 Layer 3 Skill Registry Primitive deep dive 完全 topic-overlap）+ 36.3k ⭐ 高人气（单垂直领域跑出比通用型还高的人气候）。

- **[gastownhall/gastown](./gastown-multi-agent-workspace-manager-2026.md)** — **16,330 ⭐ R668 UPDATE**（+20 in 2h from R667 16,310 ⭐, +38 in 8h from R666 16,292 ⭐, sustained strong growth），MIT，**多 Agent 工作空间编排系统 + Layer 2 Orchestrator 标杆**：Git Worktree 隔离 + Beads 账本 + Witness/Deacon/Dogs 三层监控 + v1.2.0/v1.2.1 关键稳定性修复 + v1.2.x 新增 Convoys/Molecules/Refinery 工业级 multi-agent 工作流引擎（关联：R666 deep dive [gastown-multi-agent-orchestration-deep-dive-r666-harness-protocolization-extension-2026](../orchestration/gastown-multi-agent-orchestration-deep-dive-r666-harness-protocolization-extension-2026.md) → harness 协议化三维度体系的 multi-agent 维度工业级实证）+ R667 新增 ogulcancelik/herdr 作为 Multi-Agent Stack Layer 1 (Multiplexer) 独立收敛证据 + R668 Layer 3 Skill Registry Primitive 通过 Bead-ID → Skill-Name 契约联动。**R668 UPDATE**: 距 17k⭐ BREAK 670⭐ gap，R668-R672 likely BREAK（持续增长 +20/2h）。

- **[OthmanAdi/planning-with-files](./othmanadi-planning-with-files-skill-md-23105-stars-2026.md)** — **24,647 ⭐ R668 UPDATE**（+25 in 2h from R667 24,622 ⭐, +45 in 4h from R667 24,602 ⭐, +64 in 22h from R665 24,583 ⭐, sustained strong growth），MIT，**Layer 4 State/Memory 标杆 + R661-R664 harness 协议化三维度全开最小化实证**：Markdown 计划 + checklist + 60+ Agent 跨平台 SKILL.md 共享（Claude Code / Codex CLI / Cursor / Hermes / Pi / Kiro / OpenCode）+ v3.2.0 + 186 passed tests。**R668 UPDATE**: 距 25k⭐ BREAK 仅 **353⭐ gap**，R668-R670 likely BREAK（持续增长 +25/2h）+ R668 Layer 3 Skill Registry Primitive 通过 Skill-Completion → Markdown-Checklist 契约联动。

- **[alirezarezvani/claude-skills](./alirezarezvani-claude-skills-354-production-skills-20080-stars-2026.md)** — **20,461 ⭐ R668 UPDATE**（+381 in 48h from R664 20,080 ⭐, sustained strong growth），MIT，**354 个生产级 Skills + Layer 3.2 Skill Registry 实现层跨平台标杆**：8 类职能域 marketplace（工程/产品/营销/AEO/安全/合规/C-level/学术研究/Research Ops）+ Skills/Agents/Personas 三层抽象 + BYO-sync tier 跨平台同步（Hermes Agent `sync-hermes-skills.py` + Mistral Vibe `vibe-install.sh`）+ 跨 13 个 AI Coding Tools 通用（Claude Code / OpenAI Codex / Gemini CLI / OpenClaw / Hermes Agent / Mistral Vibe / Cursor / Aider / Windsurf / Kilo Code / OpenCode / Augment / Antigravity）+ agentskills.io SKILL.md 标准协议中立 + SkillCheck Validated 认证。**R668 UPDATE**: 与 [R668 Layer 3 Skill Registry Primitive article](../orchestration/multi-agent-stack-r668-skill-registry-primitive-horizontal-decoupling-deep-dive-2026.md) 形成 Layer 3.2 Skill Registry 实现层核心实证 + 与 [marketingskills](./coreyhaines31-marketingskills-vertical-skill-registry-marketing-36347-stars-2026.md) 形成「通用型 vs 垂直型」Skill Library 双实证。

- **[ai-boost/awesome-harness-engineering](./ai-boost-awesome-harness-engineering-2026.md)** — **2,762 ⭐ R668 UPDATE**（+5 in 2h from R667 2,757 ⭐, +33 in 24h from R666 2,729 ⭐, sustained slow growth），**Harness Engineering 领域最完整的精选资源列表**：R661 合集化为「harness 协议化三维度体系」（horizontal × vertical × cross-device）。**R668 UPDATE**: **v2.0 NOT released**（持续 5 轮 R663-R668 NOT triggered），R667 Multi-Agent Stack 分层修正预测 + R668 Layer 3 Skill Registry 三子层修正预测等待 v2.0 采纳。R668 修正建议：v2.0 应将 Skill Registry Primitive 进一步拆分为 3 个 Sub-Primitive：Skills Spec + Skill Registry + Skill Library（详见 [R668 article](../orchestration/multi-agent-stack-r668-skill-registry-primitive-horizontal-decoupling-deep-dive-2026.md) 第六节）。

- **[ogulcancelik/herdr](./ogulcancelik-herdr-agent-multiplexer-rust-11903-stars-2026.md)** — **🎯 12,000 ⭐ R669 BREAK 确认！**（R668 11,950 → R669 12,000, **+50 in 2h, R667 NEW PROJECT 后的第一个 major milestone**），Rust，AGPL-3.0-or-later + 商业 dual-license，**tmux-rebuilt-for-agents —— Multi-Agent Stack Layer 1 (Multiplexer) 标杆**：per-Agent 真 PTY pane + agent-aware sidebar 状态可视化（🔴 blocked / 🟡 working / 🔵 done / 🟢 idle）+ background server detach/reattach（关笔记本不杀进程）+ socket API 让 Agent 也能驱动 multiplexer + ~10MB 静态二进制 SSH-friendly。**R669 UPDATE**: 距 13k⭐ BREAK 仅 **1000⭐ gap**，R669-R672 likely BREAK（+50/2h sustained strong growth）+ 与 [R669 Layer 4 State/Memory Primitive article](../orchestration/multi-agent-stack-r669-state-memory-primitive-learning-vs-filesystem-paradigm-2026.md) Layer 4 Cross-Layer Contract（Memory-Pane Contract）集成实证。

- **[gastownhall/gastown](./gastown-multi-agent-workspace-manager-2026.md)** — **16,345 ⭐ R669 UPDATE**（+15 in 2h from R668 16,330, +35 in 4h, +33 in 4h, sustained strong growth），MIT，**多 Agent 工作空间编排系统 + Layer 2 Orchestrator 标杆**：Git Worktree 隔离 + Beads 账本 + Witness/Deacon/Dogs 三层监控 + v1.2.1 关键稳定性修复。**R669 UPDATE**: 距 17k⭐ BREAK 655⭐ gap，R669-R672 likely BREAK（持续增长 +15/2h）+ R669 Layer 4 State/Memory Primitive 通过 State-Bead Contract（Layer 4 ↔ Layer 2）与 hindsight (Learning) + planning-with-files (Filesystem) 集成。

- **[OthmanAdi/planning-with-files](./othmanadi-planning-with-files-skill-md-23105-stars-2026.md)** — **24,665 ⭐ R669 UPDATE**（+18 in 2h from R668 24,647 ⭐, +82 in 24h, sustained strong growth），MIT，**Layer 4.2 Filesystem Paradigm 标杆 + R661-R664 harness 协议化三维度全开最小化实证**：Markdown 计划 + checklist + 60+ Agent 跨平台 SKILL.md 共享（Claude Code / Codex CLI / Cursor / Hermes / Pi / Kiro / OpenCode）+ v3.2.0 + 186 passed tests。**R669 UPDATE**: 距 25k⭐ BREAK 仅 **335⭐ gap**，R669-R670 likely BREAK（+18/2h sustained growth）+ [R669 Layer 4 State/Memory Primitive article](../orchestration/multi-agent-stack-r669-state-memory-primitive-learning-vs-filesystem-paradigm-2026.md) 将 planning-with-files 作为 **Layer 4.2 Filesystem Paradigm 标杆** 与 vectorize-io/hindsight (Layer 4.1 Learning Paradigm 18k ⭐) 形成 R669 双范式实证。

- **[coreyhaines31/marketingskills](./coreyhaines31-marketingskills-vertical-skill-registry-marketing-36347-stars-2026.md)** — **36,376 ⭐ R669 UPDATE**（+29 in 2h from R668 36,347 ⭐, sustained strong growth），Markdown + Shell，MIT，**营销垂直 Skill Library —— Multi-Agent Stack Layer 3.3 Skill Library 内容层核心实证**：12+ 类（35+ skills）覆盖 7 个营销子领域 + `product-marketing` 基础 skill + Skill 互相 cross-reference 依赖图 + 跨 Claude Code / OpenAI Codex / Cursor / Windsurf 4 Control Plane + Agent Skills spec 协议中立。**R669 UPDATE**: 距 38k⭐ BREAK 1,624⭐ gap，距 40k⭐ BREAK 3,624⭐ gap，R669-R672 likely BREAK（sustained strong growth）+ R669 Layer 4 通过 Skill-Completion → Markdown-Checklist (Memory-Skill Contract) 与 planning-with-files Layer 4.2 Filesystem Paradigm 集成。

- **[alirezarezvani/claude-skills](./alirezarezvani-claude-skills-354-production-skills-20080-stars-2026.md)** — **20,492 ⭐ R669 UPDATE**（+31 in 2h from R668 20,461 ⭐, +412 in 50h, sustained strong growth），MIT，**354 个生产级 Skills + Layer 3.2 Skill Registry 实现层跨平台标杆**：8 类职能域 marketplace + Skills/Agents/Personas 三层抽象 + BYO-sync tier 跨平台同步 + 跨 13 个 AI Coding Tools 通用。**R669 UPDATE**: 持续 monitoring + R669 Layer 4 通过 Skill-Completion → Markdown-Checklist (Memory-Skill Contract) 与 Filesystem Paradigm 集成。

- **[ai-boost/awesome-harness-engineering](./ai-boost-awesome-harness-engineering-2026.md)** — **2,765 ⭐ R669 UPDATE**（+3 in 2h from R668 2,762 ⭐, sustained slow growth），Harness Engineering 领域最完整的精选资源列表。**R669 UPDATE**: **v2.0 NOT released**（持续 6 轮 R663-R669 NOT triggered），R667 + R668 + **R669 第三轮**修正预测等待 v2.0 采纳。R669 修正建议：v2.0 应将 State/Memory Primitive 拆分为 2 个 Paradigm（Learning + Filesystem）+ 4 Cross-Paradigm Contract（详见 [R669 article](../orchestration/multi-agent-stack-r669-state-memory-primitive-learning-vs-filesystem-paradigm-2026.md) 第六节）。

- **[vectorize-io/hindsight](./vectorize-io-hindsight-agent-memory-that-learns-16216-stars-2026.md)** — **18,006 ⭐ R669 UPDATE**（R354 2026-06-13 16,216 ⭐ → R669 2026-07-06 18,006 ⭐, **+1,790 ⭐ in 23 days, +11.0% sustained strong growth**），MIT，**Layer 4.1 Learning Paradigm 标杆 + bi-temporal memory + LongMemEval SOTA**：bi-temporal memory（event_time + ingestion_time 双时态）+ 4 retrieval strategies（semantic/prompted/hybrid+BM25/GRPO RL experimental）+ Fortune 500 production + 2 行代码接入 + Oracle AI Database 企业级后端 + awesome-harness-engineering 收录到 Memory & State section（commit 73336b66, 2026-07-01）。**R669 UPDATE**: 与 [OthmanAdi/planning-with-files](./othmanadi-planning-with-files-skill-md-23105-stars-2026.md)（Layer 4.2 Filesystem Paradigm 24.6k ⭐）+ [R669 Layer 4 State/Memory Primitive article](../orchestration/multi-agent-stack-r669-state-memory-primitive-learning-vs-filesystem-paradigm-2026.md) 形成 **Layer 4.1 Learning vs Layer 4.2 Filesystem 双范式实证**（Memory That Learns vs File-based Planning 互补分工）。

- **[ogulcancelik/herdr](./ogulcancelik-herdr-12039-stars-r670-layer-1-multiplexer-2026.md)** — **12,039 ⭐ R670 UPDATE**（R669 12,000 → R670 12,039 ⭐, +39 in 2h, +0.33%/2h sustained），Rust，AGPL-3.0-or-later + 商业 dual-license，**Layer 1 Multiplexer 12k⭐ BREAK 已确认 + 13k⭐ BREAK 临界监测**：per-Agent 真 PTY pane + agent-aware sidebar 状态可视化（🔴 blocked / 🟡 working / 🔵 done / 🟢 idle）+ background server detach/reattach + socket API + ~10MB 静态二进制 SSH-friendly。**R670 UPDATE**: 距 13k⭐ BREAK **961⭐ gap**，R670-R673 likely BREAK（+30-40/day sustained strong growth）+ v0.7.1 release 稳定 + preview-2026-06-30 候选 v0.7.2 / v0.8.0 release window (R671-R673)。

- **[OthmanAdi/planning-with-files](./othmanadi-planning-with-files-24691-stars-r670-layer-4-2-filesystem-2026.md)** — **24,691 ⭐ R670 UPDATE**（R669 24,665 → R670 24,691 ⭐, +26 in 2h, +0.11%/2h sustained），MIT，**Layer 4.2 Filesystem Paradigm 25k⭐ BREAK 推进**：Markdown 计划 + checklist + 60+ Agent 跨平台 SKILL.md 共享（Claude Code / Codex CLI / Cursor / Hermes / Pi / Kiro / OpenCode）+ v3.2.0 + completion gate v3.0.0 + 96.7% pass rate benchmark + 186 passed tests。**R670 UPDATE**: 距 25k⭐ BREAK **309⭐ gap**，R671-R672 likely BREAK（+13/day sustained 推进）+ 与 [R670 Layer 4 Hybrid Memory Architecture article](../orchestration/multi-agent-stack-r670-layer-4-hybrid-memory-architecture-protocol-monitoring-2026.md) 形成 **Layer 4.2 Filesystem Paradigm** 标杆实证 + Filesystem vs Hybrid 互补分工。

- **[gastownhall/gastown](./gastown-multi-agent-workspace-manager-16363-stars-r670-layer-2-orchestrator-2026.md)** — **16,363 ⭐ R670 UPDATE**（R669 16,345 → R670 16,363 ⭐, +18 in 2h, +0.11%/2h sustained），MIT，**Layer 2 Orchestrator 17k⭐ BREAK 推进 + v1.3.0 release 候选窗口**：Git Worktree 隔离 + Beads 账本 + Witness/Deacon/Dogs 三层监控 + v1.2.1 (2026-06-06) 稳定 + Convoys/Molecules/Refinery 工业级 multi-agent 工作流引擎。**R670 UPDATE**: 距 17k⭐ BREAK **637⭐ gap**，预测 R672-R704 BREAK（if acceleration → R672-R674，if sustained → R704）+ v1.3.0 release 候选特性（Dolt SPoF 解决方案 + Mayor 高可用 + Windows 完整兼容 + herdr cross-mention）。

- **[coreyhaines31/marketingskills](./coreyhaines31-marketingskills-36412-stars-r670-layer-3-3-skill-library-2026.md)** — **36,412 ⭐ R670 UPDATE**（R669 36,376 → R670 36,412 ⭐, +36 in 2h, +0.10%/2h sustained），Markdown + Shell，MIT，**Layer 3.3 Skill Library 营销垂直 38k⭐ BREAK 推进**：12+ 类（35+ skills）覆盖 SEO & Content / CRO / Content & Copy / Paid & Measurement / Growth & Retention / Sales & GTM / Strategy 7 个营销子领域 + `product-marketing` 基础 skill + Skill 互相 cross-reference 依赖图 + 跨 4 个 Control Plane。**R670 UPDATE**: 距 38k⭐ BREAK **1,588⭐ gap**，预测 R720-R725 BREAK（+18/day sustained）+ v2.0 release 候选窗口监测（R670-R680）。

- **[vectorize-io/hindsight](./vectorize-io-hindsight-agent-memory-that-learns-18008-stars-r670-layer-4-1-learning-2026.md)** — **18,008 ⭐ R670 UPDATE**（R669 18,006 → R670 18,008 ⭐, +2 in 2h 异常缓慢, +0.01%/2h），MIT，**Layer 4.1 Learning Paradigm + SKILL.md 自带标准化触发**：bi-temporal memory + 4 retrieval strategies（semantic/prompted/hybrid+BM25/GRPO RL）+ Fortune 500 production + 2 行代码接入 + Oracle AI Database + arXiv:2512.12818。**R670 NEW 关键发现**: Hindsight 已主动发布 SKILL.md 接入 Layer 3.1 Skills Spec 标准化（`npx skills add https://github.com/vectorize-io/hindsight --skill hindsight-docs`，Works with Claude Code, Cursor, and other AI coding assistants）——**Memory-Skill Contract 1st-party 标准化首个触发证据**。**R670 UPDATE**: 距 19k⭐ BREAK **992⭐ gap**（R670 异常 +2 in 2h 延后预测至 R740-R745）+ awesome-harness-engineering 2026-07-01 commit "Add Hindsight to Memory & State section" 验证 R669 monitoring（仍归类到统一 section，未拆分 Paradigm）。

- **[alirezarezvani/claude-skills](./alirezarezvani-claude-skills-20540-stars-r670-layer-3-2-skill-registry-2026.md)** — **20,540 ⭐ R670 UPDATE**（R669 20,492 → R670 20,540 ⭐, +48 in 2h, +0.23%/2h sustained strong growth），MIT，**Layer 3.2 Skill Registry 跨 13 Control Planes 持续 monitoring**：354 个生产级 Skills + 8 类职能域 marketplace + Skills/Agents/Personas 三层抽象 + BYO-sync tier 跨平台同步（Hermes Agent + Mistral Vibe）+ agentskills.io SKILL.md 标准协议中立 + SkillCheck Validated。**R670 UPDATE**: 20k⭐ 已突破 + 距 22k⭐ **1,460⭐ gap** + BYO-sync tier 跨平台同步扩展窗口监测。

- **[ai-boost/awesome-harness-engineering](./ai-boost-awesome-harness-engineering-r670-2771-stars-2026.md)** — **2,771 ⭐ R670 UPDATE**（R669 2,765 → R670 2,771 ⭐, +6 in 2h, +0.22%/2h sustained slow growth），**Harness Engineering 领域最完整的精选资源列表**。**R670 UPDATE**: **v2.0 NOT released**（持续 6 轮 R663-R670 NOT triggered）+ commit 活跃（5 commits in 7 days）。R670 关键观察：2026-07-01 commit "Add Hindsight to Memory & State section" 验证 R669 hindsight monitoring 但**未采纳 R669 拆分 Learning vs Filesystem Paradigm 建议**（仍归类到统一 section）——这是 v2.0 修订的关键缺口。R670 修正建议：v2.0 应将 State/Memory Primitive 拆分为 3 个 Paradigm（Learning + Filesystem + Hybrid）+ 6 Cross-Paradigm Contract（详见 [R670 article](../orchestration/multi-agent-stack-r670-layer-4-hybrid-memory-architecture-protocol-monitoring-2026.md) 第九节）。距 3k⭐ BREAK **229⭐ gap**，预测 R700-R709 BREAK。
